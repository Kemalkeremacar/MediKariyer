...
export const LanguagesScreen = () => {
  ...
  const { data: languages = [], isLoading, error, refetch, isRefetching } = useQuery({
    queryKey: ['languages'],
    queryFn: () => profileService.getLanguages(),
  });

  const handleAddLanguage = () => {
    Alert.alert('Yakında', 'Dil ekleme özelliği yakında eklenecek');
  };
  ...
  return (
    <Screen scrollable={false}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <LanguageCard
            language={item.language || 'Dil'}
            level={item.level || 'Seviye'}
            onPress={() => Alert.alert('Yakında', 'Dil düzenleme özelliği yakında eklenecek')}
          />
        )}
        refreshControl={
